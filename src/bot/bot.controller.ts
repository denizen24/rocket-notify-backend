...
export class BotController {
  ...
  constructor(private readonly userService: UserService) {
    this.logger.log('✅ BotController инициализирован');
  }
...
`;
  ...
  async catchAll(@Ctx() ctx: Context) {
    this.logger.log(`🔔 [BOT-CONTROLLER] Получено обновление`);
    this.logger.log(`🔔 [BOT-CONTROLLER] Update: ${JSON.stringify(ctx.update, null, 2)}`);
    this.logger.log(`🔔 [BOT-CONTROLLER] Update type: ${ctx.updateType}`);
    this.logger.log(`🔔 [BOT-CONTROLLER] Message: ${ctx.message ? JSON.stringify(ctx.message, null, 2) : 'нет'}`);
    
    if (ctx.message && 'text' in ctx.message) {
      this.logger.log(`📱 [BOT-CONTROLLER] Получено сообщение: ${ctx.message.text}`);
    }
  }
}
