...
`rocket-notify-backend` — сервис на NestJS, который периодически опрашивает Rocket.Chat и Pachca и отправляет уведомления о росте числа непрочитанных сообщений в Telegram‑канал через телеграмм бота. Можно выбрать любой канал, главное указать его ID. Проект предназначен для фонового мониторинга, без пользовательского UI. Создан в виду того что в бесплатной версии Rocket.Chat есть ограничения по уведомлениям и мобильные клиенты не получают уведомлений из чатов.
...
- `PachcaModule` — интеграция с Pachca (API‑клиент и watcher).
...
- `PACHCA_BASE_URL` — базовый URL Pachca API.
- `PACHCA_ACCESS_TOKEN` — токен пользователя Pachca.
...
- Параметры Pachca берутся из `PACHCA_*` переменных окружения.
